...
def get_predcoll():
    ...
    for dist in range(-2,3):
        predColl.add_pred(name='dist_%d'%dist,arguments=['Y','Y'])
    ...
    def Fn():
        return DNFLayer( [5,1],sig=2., and_init=[-2,.1],or_init=[-2,1.1],or_count=20,pta=['f0(A,B)']) #,
    incp= [ p.name for p in predColl.preds]
        
    predColl.add_pred( name='up'  ,arguments=[],Frules=LOP.and_op()) \
        .add_rule(variables=[ 'N','Y',  'Y' ] , Fn=Fn, inc_preds= incp, exc_conds=[('*','rep1')],arg_funcs=['P','M'],Fvar=LOP.or_op_max, use_neg=False)  
    ...
    predColl.add_pred( name='noop'  ,arguments=[],Frules=LOP.and_op()) \
        .add_rule(variables=[ 'N','Y',  'Y' ] , Fn=Fn, inc_preds= None, exc_conds=[('*','rep1')],arg_funcs=['P', 'M'],Fvar=LOP.or_op_max, use_neg=False)   
    ...
    for i in range(DIM2):
        for j in range(DIM2):
            ...
            for dist in range(-2,3):
                if i-j == dist:
                    bg.add_backgroud('dist_%d'%dist ,('%d'%i,'%d'%j,) )     
...
optimizer = tfa.optimizers.SWA(optimizer,average_period=10)   
...
def get_logits( state, ST ):
    ...
    X,Inds = bg.make_batch( bs = tf.shape(state)[0] )
    for i in range(F_COUNT):
        X['f%d'%i] = state[:,:,i+1]
