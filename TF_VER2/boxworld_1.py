...
def get_predcoll(num_box, dim_v,dim_h):
    ...
    predColl.add_pred(name='movable'  ,arguments=['B','B']).add_fixed_rule(dnf=['not covered(A), not covered(B), not B_same(A,B), not floor(A)'],variables=[]) 


    def Fn():
        return DNFLayer( [4,1],sig=2., and_init=[-2,.1],or_init=[-2,1.1],pta=['movable(A,B)'] )#,) 
